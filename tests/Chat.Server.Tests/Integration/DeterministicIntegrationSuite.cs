...
{
    [TestClass]
    public class DeterministicIntegrationSuite
    {
        ...
        public void Test_PollingMessageDelivery() => PollingMessageDelivery(TestServerFixture.PollingUrl);

        [TestMethod]
        [TestCategory("Integration")]
        public void Test_PollingJoinBoundary() => PollingJoinBoundary(TestServerFixture.PollingUrl);

        [TestMethod]
        [TestCategory("Integration")]
        public void Test_ChannelFileVisibilityBoundary() => ChannelFileVisibilityBoundary(TestServerFixture.PollingUrl);

        [TestMethod]
        [TestCategory("Integration")]
        public void Test_DuplicateSignIn() => DuplicateSignIn(TestServerFixture.PollingUrl);

        [TestMethod]
        [TestCategory("Integration")]
        public void Test_ChannelMembershipInvariant() => ChannelMembershipInvariant(TestServerFixture.PollingUrl);

        [TestMethod]
        [TestCategory("Integration")]
        public void Test_DuplexCallbackDelivery() => DuplexCallbackDelivery(TestServerFixture.PollingUrl, TestServerFixture.DuplexUrl);

        private static void PollingMessageDelivery(string url)
        {
            var sf = PollingFactory(url);
            var rf = PollingFactory(url);
            var s = sf.CreateChannel();
            var r = rf.CreateChannel();
            const string sid = "p0_det_sender", rid = "p0_det_receiver", channel = "general", text = "p0-det-public";
            try
            {
                SignInClean(s, sid);
                SignInClean(r, rid);
                s.CreateChannel(channel);
                TestAssert.True(s.JoinChannel(sid, channel), "Sender join failed");
                TestAssert.True(r.JoinChannel(rid, channel), "Receiver join failed");
                TestAssert.False(r.GetPendingMessages(rid).Any(), "Receiver unexpectedly had messages before send");
                s.SendMessage(sid, channel, text);
                WaitUntil(() => r.GetPendingMessages(rid).Any(m => m.SenderId == sid && m.Content == text), 3000, "Receiver did not receive the public message");
            }
            finally
            {
                SignOut(s, sid);
        ...
        {
            var af = PollingFactory(url);
            var bf = PollingFactory(url);
            var a = af.CreateChannel();
            var b = bf.CreateChannel();
            const string aid = "p0_det_boundary_a", bid = "p0_det_boundary_b", channel = "general";
            try
            {
                SignInClean(a, aid);
                SignInClean(b, bid);
        ...
        private static void ChannelFileVisibilityBoundary(string url)
        {
            ...
            var a = af.CreateChannel();
            var b = bf.CreateChannel();
            const string aid = "p1_file_sender", bid = "p1_file_receiver";
            string channel = "general_" + Guid.NewGuid().ToString("N");
            try
            {
                SignInClean(a, aid);
                SignInClean(b, bid);
                a.CreateChannel(channel);
                ...
                System.Threading.Thread.Sleep(50); // Ensure system time ticks past UploadedAt before joining
                ...
                var nonMember = PollingFactory(url);
                var nc = nonMember.CreateChannel();
                const string nid = "p1_file_nonmember";
                try
                {
                    SignInClean(nc, nid);
                    TestAssert.False(nc.GetChannelFiles(nid, channel).Any(), "Non-member received channel files");
                }
                finally
                {
                    SignOut(nc, nid);
        ...
        {
            var f = PollingFactory(url);
            var c = f.CreateChannel();
            const string id = "p0_det_duplicate";
            try
            {
                SignOut(c, id);
                TestAssert.True(c.SignIn(id), "Initial sign-in failed");
                TestAssert.False(c.SignIn(id), "Duplicate sign-in was accepted");
            }
            finally
            {
                SignOut(c, id);
        ...
        private static void ChannelMembershipInvariant(string url)
        {
            var f = PollingFactory(url);
            var c = f.CreateChannel();
            const string id = "p0_det_membership";
            try
            {
                const string a = "p0_det_channel_a", b = "p0_det_channel_b";
                c.CreateChannel(a);
                c.CreateChannel(b);
                SignOut(c, id);
                TestAssert.True(c.SignIn(id), "Sign-in failed");
                TestAssert.True(c.JoinChannel(id, a), "First join failed");
                TestAssert.True(c.JoinChannel(id, b), "Second join failed");
                TestAssert.False(c.GetChannelMembers(a).Contains(id), "User remained in previous channel");
                TestAssert.True(c.GetChannelMembers(b).Contains(id), "User was not moved to second channel");
            }
            finally
            {
                SignOut(c, id);
        ...
        {
            var pf = PollingFactory(pollingUrl);
            var polling = pf.CreateChannel();
            ...
            var duplex = df.CreateChannel();
            const string id = "p0_det_duplex", channel = "general", text = "p0-det-duplex";
            try
            {
                SignOut(polling, id);
                ...
                TestAssert.Equal(text, callback.LastMessage.Content, "Duplex callback contained the wrong message");
