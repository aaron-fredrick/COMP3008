...
namespace Chat.Client.Duplex.Services
{
    public class DuplexServiceClient : IDisposable
    {
        ...
        {
            _callback = new ChatCallbackHandler(_dispatcher, this); _instanceContext = new InstanceContext(_callback);
            ...
            _channelFactory = new DuplexChannelFactory<IDuplexChatService>(_instanceContext, binding, new EndpointAddress(_serverUrl)); _proxy = _channelFactory.CreateChannel(); _isConnected = true;
        }
        ...
        public bool JoinChannel(string userId, string channelName) { try { return _proxy.JoinChannel(userId, channelName); } catch (Exception ex) { HandleError(ex); return false; } }
        public void LeaveChannel(string userId) { try { _proxy.LeaveChannel(userId); } catch (Exception ex) { HandleError(ex); } }
        public System.Collections.Generic.List<string> GetChannelMembers(string channelName) { try { return _proxy.GetChannelMembers(channelName); } catch (Exception ex) { HandleError(ex); return new System.Collections.Generic.List<string>(); } }
        public void SendMessage(string senderId, string channelName, string content) { try { _proxy.SendMessage(senderId, channelName, content); } catch (Exception ex) { HandleError(ex); } }
        public bool SendPrivateMessage(string senderId, string recipientId, string content) { try { return _proxy.SendPrivateMessage(senderId, recipientId, content); } catch (Exception ex) { HandleError(ex); return false; } }
        public bool ShareFile(string uploaderId, string channelName, string fileName, FileType fileType, byte[] fileData) { try { return _proxy.ShareFile(uploaderId, channelName, fileName, fileType, fileData); } catch (Exception ex) { HandleError(ex); return false; } }
        public SharedFile SharePrivateFile(string senderId, string recipientId, string fileName, FileType fileType, byte[] fileData) { try { return _proxy.SharePrivateFile(senderId, recipientId, fileName, fileType, fileData); } catch (Exception ex) { HandleError(ex); return null; } }
        public SharedFile GetFile(string userId, Guid fileId) { try { return _proxy.GetFile(userId, fileId); } catch (Exception ex) { HandleError(ex); return null; } }
        ...
        private void HandleError(Exception ex)
        {
            ...
            if (_isConnected) { _isConnected = false; _dispatcher.BeginInvoke(new Action(() => ConnectionLost?.Invoke(this, EventArgs.Empty))); }
        }
        public void Dispose()
        {
            ...
            if (_instanceContext != null) _instanceContext.Close();
        }
        internal void OnMessageReceivedInternal(Message message) { MessageReceived?.Invoke(this, message); }
        internal void OnPrivateMessageReceivedInternal(Message message) { PrivateMessageReceived?.Invoke(this, message); }
        internal void OnFileSharedInternal(SharedFile file) { FileShared?.Invoke(this, file); }
        internal void OnPrivateFileSharedInternal(SharedFile file) { PrivateFileShared?.Invoke(this, file); }
        internal void OnChannelListChangedInternal() { ChannelListChanged?.Invoke(this, EventArgs.Empty); }
        internal void OnChannelMembersChangedInternal(string channelName) { ChannelMembersChanged?.Invoke(this, channelName); }
        internal void OnUserDisconnectedInternal(string userId) { UserDisconnected?.Invoke(this, userId); }
    }
}