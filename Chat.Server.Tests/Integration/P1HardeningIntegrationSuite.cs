...
namespace Chat.Server.Tests.Integration
{
    internal static class P1HardeningIntegrationSuite
    {
        ...
        private static void ChannelFileDownloadAuthorization(string url)
        {
            ...
            var nf = PollingFactory(url);
            var a = af.CreateChannel();
            var b = bf.CreateChannel();
            var n = nf.CreateChannel();
            const string aid = "p1_auth_file_sender";
            const string bid = "p1_auth_file_member";
            const string nid = "p1_auth_file_nonmember";
            ...
            try
            {
                ...
                var visible = b.GetChannelFiles(bid, channel);
                var metadata = visible.FirstOrDefault(f => f.FileName == "p1-authorized.txt");
                ...
                TestAssert.True(n.JoinChannel(nid, "p1-auth-other"), "Non-member setup channel join failed");
            ...
            {
                SignOut(a, aid); SignOut(b, bid); SignOut(n, nid);
                Abort(a, af); Abort(b, bf); Abort(n, nf);
            }
        }
        ...
        private static void PrivateMessageSameChannelAuthorization(string url)
        {
            ...
            var of = PollingFactory(url);
            var s = sf.CreateChannel();
            var r = rf.CreateChannel();
            var o = of.CreateChannel();
            const string sid = "p1_pm_sender";
            const string rid = "p1_pm_recipient";
            const string oid = "p1_pm_other";
            try
            {
                ...
                var pending = r.GetPendingPrivateMessages(rid);
                TestAssert.True(pending.Any(m => m.SenderId == sid && m.Content == "same-channel"), "Recipient did not receive the same-channel private message");
            ...
            {
                SignOut(s, sid); SignOut(r, rid); SignOut(o, oid);
                Abort(s, sf); Abort(r, rf); Abort(o, of);
            }
        }
        ...
        private static void PrivateFileSameChannelAuthorization(string url)
        {
            ...
            var r = rf.CreateChannel();
            var o = of.CreateChannel();
            const string sid = "p1_private_file_sender";
            const string rid = "p1_private_file_recipient";
            const string oid = "p1_private_file_other";
            try
            {
                SignInClean(s, sid); SignInClean(r, rid); SignInClean(o, oid);
                ...
                TestAssert.True(r.GetPrivateFile(rid, file.FileId) != null, "Recipient could not retrieve private file");
        ...
        private static void PrivateFilePendingDelivery(string url)
        {
            ...
            var r = rf.CreateChannel();
            var o = of.CreateChannel();
            const string sid = "p1_pending_file_sender";
            const string rid = "p1_pending_file_recipient";
            const string oid = "p1_pending_file_other";
            try
            {
                SignInClean(s, sid); SignInClean(r, rid); SignInClean(o, oid);
        ...
        private static void DuplexFileNotification(string pollingUrl, string duplexUrl)
        {
            ...
            const string id = "p1_duplex_file";
            const string channel = "p1-duplex-file";
            try
            {
                SignOut(polling, id);
        ...
        private static ChannelFactory<IChatService> PollingFactory(string url)
        {
            return new ChannelFactory<IChatService>(new BasicHttpBinding(), new EndpointAddress(url));
        }

        private static void SignInClean(IChatService client, string id)
        {
            SignOut(client, id);
            TestAssert.True(client.SignIn(id), "Sign-in failed for " + id);
        }

        private static void SignOut(IChatService client, string id)
        {
            try { client.SignOut(id); } catch { }
        }
