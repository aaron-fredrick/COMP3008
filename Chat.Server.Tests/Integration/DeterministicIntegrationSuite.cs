...
namespace Chat.Server.Tests.Integration
{
    internal static class DeterministicIntegrationSuite
    {
        ...
        {
            var sf = PollingFactory(url); var rf = PollingFactory(url);
            var s = sf.CreateChannel(); var r = rf.CreateChannel();
            const string sid = "p0_det_sender", rid = "p0_det_receiver", channel = "general", text = "p0-det-public";
            try
            {
                SignInClean(s, sid); SignInClean(r, rid);
        ...
        {
            var af = PollingFactory(url); var bf = PollingFactory(url);
            var a = af.CreateChannel(); var b = bf.CreateChannel();
        ...
        private static void DuplicateSignIn(string url)
        {
            ...
            try { SignOut(c, id); TestAssert.True(c.SignIn(id), "Initial sign-in failed"); TestAssert.False(c.SignIn(id), "Duplicate sign-in was accepted"); }
            finally { SignOut(c, id); Abort(c, f); }
        }
        ...
        private static void ChannelMembershipInvariant(string url)
        {
            ...
            try
            {
                ...
                c.CreateChannel(a); c.CreateChannel(b); SignOut(c, id);
        ...
        private static void DuplexCallbackDelivery(string pollingUrl, string duplexUrl)
        {
            ...
            var duplex = df.CreateChannel(); const string id = "p0_det_duplex", channel = "general", text = "p0-det-duplex";
            try
            {
                SignOut(polling, id);
        ...
        private static void SignInClean(IChatService c, string id)
        {
            SignOut(c, id);
            TestAssert.True(c.SignIn(id), "Sign-in failed for " + id);
        }
        ...
        private static void WaitUntil(Func<bool> condition, int timeoutMs, string message)
        {
            ...
            while (DateTime.UtcNow < end) { if (condition()) return; Thread.Sleep(50); }
