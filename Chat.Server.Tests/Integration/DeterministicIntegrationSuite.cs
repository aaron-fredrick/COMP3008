...
namespace Chat.Server.Tests.Integration
{
    internal static class DeterministicIntegrationSuite
    {
        public static int Run(string pollingUrl, string duplexUrl)
        {
            ...
            Console.WriteLine(); Console.WriteLine($"Structured integration tests: {passed} passed, {failed} failed"); return failed == 0 ? 0 : 1;
        }
        ...
        { var sf = PollingFactory(url); var rf = PollingFactory(url); var s = sf.CreateChannel(); var r = rf.CreateChannel(); const string sid = "p0_det_sender", rid = "p0_det_receiver", channel = "general", text = "p0-det-public"; try { SignInClean(s, sid); SignInClean(r, rid); s.CreateChannel(channel); TestAssert.True(s.JoinChannel(sid, channel), "Sender join failed"); TestAssert.True(r.JoinChannel(rid, channel), "Receiver join failed"); TestAssert.False(r.GetPendingMessages(rid).Any(), "Receiver unexpectedly had messages before send"); s.SendMessage(sid, channel, text); WaitUntil(() => r.GetPendingMessages(rid).Any(m => m.SenderId == sid && m.Content == text), 3000, "Receiver did not receive the public message"); } finally { SignOut(s, sid); SignOut(r, rid); Abort(s, sf); Abort(r, rf); } }
        private static void PollingJoinBoundary(string url)
        { var af = PollingFactory(url); var bf = PollingFactory(url); var a = af.CreateChannel(); var b = bf.CreateChannel(); const string aid = "p0_det_boundary_a", bid = "p0_det_boundary_b", channel = "general"; try { SignInClean(a, aid); SignInClean(b, bid); TestAssert.True(a.JoinChannel(aid, channel), "First join failed"); a.SendMessage(aid, channel, "p0-before-join"); TestAssert.True(b.JoinChannel(bid, channel), "Second join failed"); var beforeJoin = b.GetPendingMessages(bid); TestAssert.False(beforeJoin.Any(m => m.Content == "p0-before-join"), "Pre-join message was replayed"); a.SendMessage(aid, channel, "p0-after-join"); Thread.Sleep(100); var afterJoin = b.GetPendingMessages(bid); TestAssert.True(afterJoin.Any(m => m.Content == "p0-after-join"), "Post-join message was not delivered"); } finally { SignOut(a, aid); SignOut(b, bid); Abort(a, af); Abort(b, bf); } }
        private static void ChannelFileVisibilityBoundary(string url)
        {
            ...
            try
            {
                SignInClean(a, aid); SignInClean(b, bid); TestAssert.True(a.JoinChannel(aid, channel), "File sender join failed");
                TestAssert.True(a.ShareFile(aid, channel, "p1-before-join.txt", FileType.Txt, new byte[] { 1 }), "Pre-join file share failed");
                TestAssert.True(b.JoinChannel(bid, channel), "File receiver join failed");
                var beforeFiles = b.GetChannelFiles(bid, channel); TestAssert.False(beforeFiles.Any(f => f.FileName == "p1-before-join.txt"), "Pre-join file was visible");
                TestAssert.True(a.ShareFile(aid, channel, "p1-after-join.txt", FileType.Txt, new byte[] { 2 }), "Post-join file share failed");
        ...
        private sealed class TestCallback : IChatCallback { public Message LastMessage { get; private set; } public void OnChannelListChanged() { } public void OnChannelMembersChanged(string channelName) { } public void OnMessageReceived(Message message) { LastMessage = message; } public void OnPrivateMessageReceived(Message message) { } public void OnFileShared(SharedFile file) { } public void OnPrivateFileShared(SharedFile file) { } public void OnUserDisconnected(string userId) { } }
    }
}